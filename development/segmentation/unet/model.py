...
def unet(n_ch = 3,patch_height = 450,patch_width = 600,pretrained_weights = None):
	concat_axis = 3

	inp = Input((patch_height, patch_width, n_ch))

	conv1 = Conv2D(32, (3, 3), padding="same", name="conv1_1", activation="relu", data_format="channels_last")(inp)
	...
	ch, cw = get_crop_shape(inp, conv9)
	...
	#Sizing Issues, unintelligentable guesses as to which code to comment or keep
	#flatten =  Flatten()(conv9)
	#Dense1 = Dense(512, activation='relu')(flatten)
	#BN =BatchNormalization() (Dense1)
	#Dense2 = Dense(17, activation='sigmoid')(BN)

	model = Model(inputs=inp, output=conv10)
